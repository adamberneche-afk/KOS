...
//                        "PENDING USER APPROVAL" stub. Reports one
//                        review's full state via
//                        getCouncilReviewStatus() (6_Governance.gs):
//                        which cogs have verdicted, which are still
//                        outstanding, any duplicate or misspelled cog
//                        names, and the compiled halt verdict. The
//                        stimulus document itself is generated by that
//                        same file's triggerSevenBridgesReview(), not
//                        by this function.
//
  ...
  try {
    ...
    ui.createMenu('🧠 KOS v8.0')
      ...
      .addSubMenu(ui.createMenu('📊 Diagnostics')
        ...
        .addItem('Seven Bridges — Status & Verdict', 'sevenBridgesReview'))
...
function sevenBridgesReview() {
  const ui = _getUi();
  const r  = ui.prompt('🌉 Seven Bridges — Review Status',
    'Enter the Council ID to check (shown when you ran "Run full ' +
  ...
  // getCouncilReviewStatus() wraps compileCouncilVerdict_() and adds the
  // half it structurally cannot answer: which cogs have NOT verdicted yet.
  // A review is fanned out by hand over hours or days, so "who am I still
  // waiting on" is the question actually being asked here most of the time
  // — the compiled verdict only matters once everyone has voted.
  const result = getCouncilReviewStatus(councilId);
  if (!result.success) {
  ...
  if (result.received.length) {
    lines.push('RECORDED (' + result.received.length + ' of ' + result.expected + '):');
    result.received.forEach(v => lines.push(
      '  • ' + v.cog + ': ' + v.status +
      (v.count > 1 ? '  ⚠ ' + v.count + ' submissions' : '') +
      (v.summary ? '\n      ' + v.summary : '')
    ));
  }
  if (result.pending.length) {
    lines.push('', 'STILL WAITING ON:');
    result.pending.forEach(c => lines.push('  • ' + c));
  }
  if (result.unrecognized.length) {
    // Most likely a typo in the free-text Cog field. Worth calling out
    // loudly: the verdict still counts toward the halt threshold, while
    // the persona it was meant to be still reads as never having voted.
    lines.push('', 'UNRECOGNIZED COG NAMES (typo? counted anyway):');
    result.unrecognized.forEach(v => lines.push('  • "' + v.cog + '": ' + v.status));
  }
