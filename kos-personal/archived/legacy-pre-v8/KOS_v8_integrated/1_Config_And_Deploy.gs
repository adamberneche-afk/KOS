...
// ─────────────────────────────────────────────────────────────
// • resetProperties() removed — canonical version in File 5.
// • KOS_TRIGGERS expanded: runMatrixTurnstile, runHeartbeat,
//   onGovernanceEdit now auto-installed by setupAllTriggers().
// • deployFullSystem() Step 1.5: initializeKOS() provisions
//   SMP-002 v5.6 zone folders before spreadsheet creation.
// • deployFullSystem() final step: buildSystemPrompt() and
//   buildSiloPrompt() write Studio prompt docs to ACTIVE zone.
// • _copyPersonas(): missing persona scaffolds a placeholder
//   doc instead of silently skipping.
// • teardownAllTriggers(): includes new trigger names.
// • CFG unchanged — all existing constants preserved.
// ================================================================
...
const CFG = {
  ...
  INBOUND_SESSIONS_FOLDER:  '03.5_INBOUND_SESSIONS',
  ...
  EXTERNAL_TELEMETRY_SHEET: 'EXTERNAL_TELEMETRY',
...
function deployFullSystem() {
  ...
  try {
    ...
    // and 95_CE-SCRAP per SMP-002 v5.6. Idempotent.
    ...
    const sheetNames = [
      ...
      CFG.ERROR_LOG_SHEET,
      'DRIP_QUARANTINE',          // Sprint: drip schema violations
    ];
    ...
    // ── 7. Identity Key ────────────────────────────────────────
    // NOTE: generates a placeholder key from boilerplate thesis text.
    // The real key is regenerated at end of runSocraticOnboarding()
    // after _seedCoreThesisDoc() populates operator answers.
    emit('Generating placeholder Identity Key…');
    try {
      generateIdentityKey();
      emit('✔ Identity Key generated (placeholder — will be re-generated after onboarding)');
    ...
    // ── 10. Studio Prompt Documents ───────────────────────────
    // Builds the system prompt and silo council prompt docs that
    // Studio references for every inference run.
    emit('Building Studio prompt documents…');
    try {
      const sp = buildSystemPrompt();
      emit(sp.success
        ? '  ✔ System prompt: ' + sp.docUrl
        : '  ⚠ System prompt failed: ' + sp.message);
      const sl = buildSiloPrompt();
      emit(sl.success
        ? '  ✔ Silo prompt: '   + sl.docUrl
        : '  ⚠ Silo prompt failed: '   + sl.message);
    } catch (e) { fail('buildPrompts', e); }

    // ── 11. Log to ERROR_LOG sheet ────────────────────────────
    try {
...
/**
 * Installs all background triggers. Idempotent.
 *
 * Triggers installed:
...
function setupAllTriggers() {
  ...
  const KOS_TRIGGERS = [
    ...
    'onGovernanceEdit',         // Blackboard mutation trigger
    'runMatrixTurnstile',       // File 10 — Turnstile flow controller
    'runHeartbeat',             // File 10 — SMP-002 zone heartbeat
  ];
  ...
  // SMP-002 zone integrity check + 14-day Planning Buffer staleness check.
  tryInstall('runHeartbeat', () =>
    ScriptApp.newTrigger('runHeartbeat')
      .timeBased().atHour(7).everyDays(1).create()
  );

  if (installed.length > 0) log.push('  ✔ Triggers installed: ' + installed.join(', '));
...
function _buildFolderTree() {
  ...
  const f03_in  = _getOrCreateFolder('03.5_INBOUND_SESSIONS',    f03);
...
function _registerAllProperties(folders, ss) {
  ...
  const map = {
    ...
    'ID_03_5_INBOUND_SESSIONS': folders.f03_in,
...
function setupRoutingProperties() {
  ...
  const map = {
    ...
    'ID_03_5_INBOUND_SESSIONS': fetchId('03.5_INBOUND_SESSIONS',     true),
  ...
  console.log('[setupRoutingProperties] ' + ok + ' registered, ' + miss + ' missing.');
  return { ok, miss };
}
...
function _coldEngineGate(callerFunction, tier) {
  ...
  if (tier === 'TIER_2') {
    const err = new Error(
      ...
      'Complete Socratic Onboarding via the web app.'
    );
...
function _createAllFoundationDocs(folders) {
  ...
  _scaffoldDoc('SYSTEM_TELEMETRY', folders.f03, [
    ...
    { h2: 'Architecture' },    { p: 'Headless Pub/Sub. Sensors: SESSION_LOG, COG_EXHAUST (doPost), EXTERNAL_DATA (onChange).' },
  ...
  _scaffoldDoc('KOS_v8_QUICK_START', folders.f01, [
    ...
    { h2: '2. Arm the Engine' },
    { p:  'Open the web app URL → Diagnostics tab → Socratic Onboarding.' },
    ...
    { h2: '5. Governance Mutations' },
    { p:  'BRAIN_TRUST_INDEX → Blackboard tab. Set Deploy_Trigger = TRUE to fire applyMutation().' },
  ]);
...
function _copyPersonas(f02) {
  ...
  CFG.PERSONAS.forEach(baseName => {
    try {
      ...
      if (!sourceFile) {
        // Scaffold a placeholder so Studio always has a doc to open,
      ...
      // getText/setText: copies content as plain text. Formatting is not
      // preserved. If Studio reads persona docs as formatted Docs, consider
      // using DriveApp.getFileById(sourceFile.getId()).makeCopy() instead.
      ...
      newDoc.saveAndClose();
      DriveApp.getFileById(newId).moveTo(f02);
...
// ADMIN
// ================================================================
// resetProperties() is defined in 5_Error_And_Utilities.gs (canonical).
