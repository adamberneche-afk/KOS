...
function _routeVectorWeightsInternal(pd, sessionUid, timestamp) {
  try {
    ...
    // in this session's unknown weights, patch the row now.
...
function _writeMatrixRow(sheet, known, sessionUid, timestamp) {
  ...
  // Guard: if the sheet has no header columns beyond the first two,
  // it was likely created without the correct schema (e.g. manual
  // intervention or a fresh sheet that missed initialisation).
  // Writing a malformed row here would silently corrupt the matrix.
