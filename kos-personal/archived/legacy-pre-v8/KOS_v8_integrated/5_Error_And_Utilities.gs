 ...
 *
 * The timestamp component makes every call produce a unique ID even
 * for identical payloads submitted at different times. The hash
 * component makes IDs for identical payloads submitted within the
 * same millisecond differ only if the text differs.
 *
 * DEDUP NOTE: The full UID string ("LOG-{ts}-{hash}") is the dedup
 * key — not just the hash suffix. The duplicate check in
 * submitSessionLog uses existingUids.includes(logUUID), which
 * compares the complete UID. Do not shorten the UID or the dedup
 * guard will stop working.
 *
...
function _getSystemAsset(name, propKey, isFolder) {
  ...
  // Name-based fallback: filter out trashed results and warn on duplicates.
  // DriveApp.getFilesByName returns ALL files with that name including Trash
  // and files from aborted deploys. it.next() is non-deterministic when
  // multiple non-trashed results exist — always use ID from PropertiesService
  // to avoid routing to the wrong asset.
  const it = isFolder ? DriveApp.getFoldersByName(name) : DriveApp.getFilesByName(name);
  const candidates = [];
  while (it.hasNext()) {
    const candidate = it.next();
    try {
      // getIsTrashed() is only available on File objects, not Folder.
      // For folders, assume non-trashed (Drive doesn't expose this cheaply).
      if (!isFolder && DriveApp.getFileById(candidate.getId()).isTrashed()) continue;
  ...
  if (candidates.length > 1) {
    // SEC-001: Multiple non-trashed assets with the same name.
    // Use the first result but flag the operator — they need to delete
    // or rename the duplicate to prevent non-deterministic routing.
    _reportError(
      '_getSystemAsset:DUPLICATE:' + name,
      new Error(
        'Multiple non-trashed assets named "' + name + '" found in Drive (' +
        candidates.length + ' results). Using the first result (' +
        candidates[0].getId() + '). Delete or rename duplicates and re-run ' +
        'deployFullSystem() to lock the correct ID.'
      ),
      null
    );
  }
 ...
 *
 * NOTE: All returned values are strings (e.g. '0.92'), not numbers.
 * PropertiesService stores and returns strings only. All consumers
 * must call parseFloat() before arithmetic comparisons. The type is
 * intentional — do not convert to numbers here, as setProperties()
 * will coerce them back to strings on round-trip anyway.
 */
...
function _advanceOnboardingDay() {
  ...
  // Calendar-day guard: only advance once per calendar day.
  // Prevents a busy operator from burning through multiple
  // "onboarding days" in a single afternoon of active use.
  const today    = Utilities.formatDate(new Date(), Session.getScriptTimeZone(), 'yyyy-MM-dd');
  const lastDate = props.getProperty('KOS_ONBOARDING_LAST_DATE') || '';
  if (lastDate === today) return;

  props.setProperty(CFG.PROP.ONBOARDING_DAY,       String(cur + 1));
  props.setProperty('KOS_ONBOARDING_LAST_DATE',     today);
...
function archiveStagingPipeline() {
  try {
    ...
    // Two-tier terminal status check.
    // Exact-match keywords: bare status values that are complete strings.
    // Prefix-match keywords: error strings that carry a trailing message
    //   e.g. 'INTAKE_ERROR: Doc not found', 'ERROR: No File_ID'.
    // Keeping these separate prevents future statuses like 'INTAKE_PENDING'
    // from being accidentally archived by the prefix check.
    const terminalExact  = new Set([
      'PROCESSED', 'INTAKE_PROCESSED', 'PARTITIONED',
      'CONSOLIDATED', 'FAILED_PARSE', 'PHASE_2_ERROR',
    ]);
    const terminalPrefix = ['INTAKE_ERROR', 'ERROR'];

    const _isTerminal = status => {
      if (terminalExact.has(status)) return true;
      return terminalPrefix.some(p => status.startsWith(p + ':') || status === p);
    };
    ...
    for (let i = data.length - 1; i >= 1; i--) {
      const rowStatus = String(data[i][CFG.STAGING_COLS.STATUS]).trim();
      if (_isTerminal(rowStatus)) {
...
function resetProperties() {
  ...
  [
    ...
    'KOS_ONBOARDING_LAST_DATE',      // calendar-day guard for onboarding counter
    'KOS_SENSOR3_LAST_ROW',          // cursor for sensor3 incremental scan
