...
function runSocraticOnboarding() {
  ...
  // Clear the calendar-day guard so the first post-onboarding queue run
  // can advance to day 2. Without this, the guard would block advancement
  // if onboarding completes and a session is processed on the same calendar day.
  props.deleteProperty('KOS_ONBOARDING_LAST_DATE');
