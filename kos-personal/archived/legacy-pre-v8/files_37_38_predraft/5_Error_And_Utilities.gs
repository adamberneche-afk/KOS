 ...
 * CFG.MAX_CHUNK_SIZE characters.
 *
 * FIX-04: Added plain-text fallback for logs without KOS delimiters.
 * Strategy (priority order):
 *   1. If CFG.DELIMITER found → split on delimiter (structured KOS logs).
 *   2. If text > MAX_CHUNK_SIZE with no delimiter → split on paragraph
 *      breaks (\n\n). Handles plain-text pastes from non-KOS sources.
 *   3. If a single paragraph exceeds MAX_CHUNK_SIZE → word-boundary split.
 *   4. Hard split at MAX_CHUNK_SIZE as last resort.
 *
 * @param  {string}   text  Raw session log text.
 * @returns {string[]}      Array of chunks, each ≤ MAX_CHUNK_SIZE chars.
 */
function _semanticChunker(text) {
  const MAX = CFG.MAX_CHUNK_SIZE;

  // ── Strategy 1: delimiter-based (structured KOS session logs) ──
  if (text.includes(CFG.DELIMITER)) {
    ...
    splits.forEach((s, i) => {
      ...
      const block = (i === 0 && !text.startsWith(CFG.DELIMITER))
        ? s : CFG.DELIMITER + s;
      if ((cur.length + block.length) > MAX) {
  ...
  // ── Strategy 2: paragraph-break splitting (plain text) ─────────
  ...
  if (paragraphs.length > 1) {
    ...
    paragraphs.forEach(p => {
      const block = p.trim();
      if (!block) return;
      if ((cur.length + block.length + 2) > MAX) {
        if (cur) chunks.push(cur.trim());
        if (block.length > MAX) {
          _splitAtWordBoundary(block, MAX).forEach(c => chunks.push(c));
          cur = '';
        } else {
          cur = block;
        }
  ...
  // ── Strategy 3/4: word-boundary / hard split ───────────────────
  return _splitAtWordBoundary(text, MAX);
}


/**
 * Splits a string at word boundaries as close to maxLen as possible.
 * Falls back to a hard split at maxLen if no word boundary is found
 * within the last 200 characters of the window.
 *
 * @param  {string}   text    Text to split.
 * @param  {number}   maxLen  Maximum characters per chunk.
 * @returns {string[]}
 */
function _splitAtWordBoundary(text, maxLen) {
  const chunks = [];
  let   pos    = 0;
  while (pos < text.length) {
    ...
    const window    = text.substring(pos, pos + maxLen);
    const lastBreak = Math.max(window.lastIndexOf(' '), window.lastIndexOf('\n'));
    const cutAt     = lastBreak > (maxLen - 200) ? lastBreak : maxLen;
    chunks.push(text.substring(pos, pos + cutAt).trim());
    pos += cutAt;
...
// ================================================================
// SHADOW MATRIX — AMBIENT ONBOARDING ENGINE
// ================================================================

/**
 * Returns the current shadow matrix from PropertiesService.
 * Initialises all 5 shadow questions at 0 confidence if not set.
 *
 * Shadow matrix shape:
 *   { [question_key]: {
 *       inferred_value: string,
 *       confidence:     0.0–1.0,
 *       status:         'UNKNOWN' | 'HYPOTHESIZED' | 'VERIFIED',
 *       evidence_count: int,
 *       last_updated:   ISO timestamp
 *   } }
 *
 * @returns {Object} Shadow matrix keyed by CFG.SHADOW_QUESTION_KEYS.
 */
function _getShadowMatrix() {
  const props = PropertiesService.getScriptProperties();
  const raw   = props.getProperty(CFG.SHADOW_MATRIX_KEY);
  if (!raw) {
    const matrix = {};
    CFG.SHADOW_QUESTION_KEYS.forEach(k => {
      matrix[k] = { inferred_value: '', confidence: 0.0,
                    status: 'UNKNOWN', evidence_count: 0, last_updated: '' };
    });
    return matrix;
  }
  try { return JSON.parse(raw); }
  catch (e) {
    console.error('[_getShadowMatrix] Corrupt data: ' + e.message);
    return {};
  }
}
...
/**
 * Updates confidence intervals in the shadow matrix based on
 * alignment_observations in the inference payload (drip branch 9).
 *
 * Expected observations shape:
 *   { admin_ghost_signal:       string | null,
 *     relational_signal:        string | null,
 *     necessary_struggle_signal: string | null,
 *     prime_directive_signal:    string | null,
 *     temporal_signal:           string | null,
 *     confidence_deltas: {
 *       admin_ghost: 0.05, relational_targets: 0.10, ... } }
 *
 * When a question reaches VERIFIED (≥ SHADOW_VERIFY_THRESHOLD),
 * its inferred value is auto-populated into the corresponding
 * operator property if not already set by explicit onboarding.
 *
 * @param {Object} observations  From pd.alignment_observations.
 * @param {string} uid           Session UID for logging.
 */
function _updateShadowMatrix(observations, uid) {
  if (!observations) return;
  const matrix  = _getShadowMatrix();
  ...
  CFG.SHADOW_QUESTION_KEYS.forEach(key => {
    if (!matrix[key]) {
      matrix[key] = { inferred_value: '', confidence: 0.0,
                      status: 'UNKNOWN', evidence_count: 0, last_updated: '' };
    }
    const delta = parseFloat(deltas[key]) || 0;
    if (delta === 0) return;

    const entry = matrix[key];
    entry.confidence    = parseFloat(Math.min(1.0, Math.max(0.0, (entry.confidence || 0) + delta)).toFixed(3));
    entry.evidence_count = (entry.evidence_count || 0) + 1;
    entry.last_updated  = ts;
    entry.status        = entry.confidence >= (CFG.SHADOW_VERIFY_THRESHOLD || 0.75) ? 'VERIFIED'
                        : entry.confidence > 0.1 ? 'HYPOTHESIZED' : 'UNKNOWN';

    const sigKey = signalKeys[key];
    if (sigKey && observations[sigKey]) {
      entry.inferred_value = String(observations[sigKey]);
    }
  });

  // Auto-populate verified answers into operator properties
  _persistVerifiedShadowAnswers(matrix);

  PropertiesService.getScriptProperties()
    .setProperty(CFG.SHADOW_MATRIX_KEY, JSON.stringify(matrix));
  console.log('[ShadowMatrix] Updated from session ' + uid);
}


/**
 * Writes VERIFIED shadow matrix answers to their corresponding
 * operator property keys if not already explicitly set.
 * Called by _updateShadowMatrix after each confidence update.
 *
 * @param {Object} matrix  Current shadow matrix.
 */
function _persistVerifiedShadowAnswers(matrix) {
  const props  = PropertiesService.getScriptProperties();
  const propMap = {
    ...
    'prime_directive':    CFG.PROP.OPERATOR_ROLE,
    'temporal_constraints': null,
  };
  Object.entries(propMap).forEach(([shadowKey, propKey]) => {
    if (!propKey) return;
    const entry = matrix[shadowKey];
    if (entry && entry.status === 'VERIFIED' && entry.inferred_value) {
      if (!props.getProperty(propKey)) {
        props.setProperty(propKey, entry.inferred_value);
        console.log('[ShadowMatrix] Auto-populated ' + propKey + ' from VERIFIED inference.');
      }
    }
  });
}
...
/**
 * Returns shadow matrix status for the web app Diagnostics tab.
 *
 * Called via:
 *   google.script.run.withSuccessHandler(fn).getShadowMatrixStatus()
 *
 * @returns {Object} { success, questions[], overall_confidence, engine_mode }
 */
function getShadowMatrixStatus() {
  try {
    const matrix    = _getShadowMatrix();
    const questions = CFG.SHADOW_QUESTION_KEYS.map(k => {
      const e = matrix[k] || {};
      return {
        key:        k,
        ...
        evidence:   e.evidence_count || 0,
        inferred:   e.inferred_value || '',
      };
    });
    const overall = questions.reduce((s, q) => s + q.confidence, 0) / questions.length;
    return {
      success:             true,
      questions,
      overall_confidence:  parseFloat(overall.toFixed(2)),
      all_verified:        questions.every(q => q.status === 'VERIFIED'),
      engine_mode:         overall >= 0.75 ? 'CALIBRATED' : overall > 0.3 ? 'LEARNING' : 'COLD',
    };
  } catch (e) {
...
// ================================================================
// DAILY PRIMER — TIME-DRIVEN TRIGGER
// ================================================================

/**
 * Generates a daily session starter document from current vector
 * state, shadow matrix status, and operator properties. Saved to
 * 03.1_CURRENT_STATE as DAILY_PRIMER_YYYY-MM-DD.
 *
 * Fires: daily at 06:00 via time-driven trigger (setupAllTriggers).
 * Also callable from the web app Diagnostics tab.
 */
function generateDailyPrimer() {
  try {
    ...
    // Build vector summary from last VECTOR_MATRIX row
    let vectorSummary = '  No sessions processed yet.';
    if (matrix && matrix.getLastRow() > 1) {
      const headers    = matrix.getRange(1, 1, 1, matrix.getLastColumn()).getValues()[0];
      const lastRow    = matrix.getRange(matrix.getLastRow(), 1, 1, matrix.getLastColumn()).getValues()[0];
      const themeStart = 2;
      vectorSummary = headers.slice(themeStart)
        .filter(h => h !== 'INCUBATOR_SIGNALS')
        .map((t, i) => '  ' + String(t).padEnd(22) + parseFloat(lastRow[themeStart + i] || 0).toFixed(2))
        .join('\n');
    }

    // Shadow matrix summary
    const shadow        = _getShadowMatrix();
    const shadowSummary = CFG.SHADOW_QUESTION_KEYS.map(k => {
      const e = shadow[k] || {};
      return '  ' + k.padEnd(24) +
             (e.status || 'UNKNOWN').padEnd(14) +
             Math.round((e.confidence || 0) * 100) + '%';
    }).join('\n');

    const role    = props.getProperty(CFG.PROP.OPERATOR_ROLE)       || '[ Not yet inferred ]';
    const day     = props.getProperty(CFG.PROP.ONBOARDING_DAY)      || '?';
    const vision  = props.getProperty(CFG.PROP.VISION_90_DAY)       || '[ Not yet inferred ]';
    const targets = props.getProperty(CFG.PROP.RELATIONAL_TARGETS)  || '[ Not yet inferred ]';

    const primerContent = [
      '[🧠 RTP — DAILY SESSION PRIMER]',
      'Generated: ' + ts + '  |  KOS v' + CFG.SYSTEM_VERSION,
      'Role: ' + role + '  |  Onboarding Day: ' + day + ' / ' + CFG.ONBOARDING_DAYS,
      '',
      '═'.repeat(52),
      '',
      '── VECTOR STATE (CALIBRATED WEIGHTS) ───────────────────',
      vectorSummary,
      '',
      '── SHADOW MATRIX (AMBIENT CALIBRATION) ─────────────────',
      shadowSummary,
      '',
      '── 90-DAY VISION ────────────────────────────────────────',
      '  ' + vision,
      ...
      '',
      '═'.repeat(52),
      '[Paste this block at the start of a new Studio session]',
    ].join('\n');

    // Save to 03.1_CURRENT_STATE
    const today     = Utilities.formatDate(new Date(), Session.getScriptTimeZone(), 'yyyy-MM-dd');
    const docName   = 'DAILY_PRIMER_' + today;
    ...
    if (folder) {
      const existing = folder.getFilesByName(docName);
      if (existing.hasNext()) {
        const doc = DocumentApp.openById(existing.next().getId());
        doc.getBody().setText(primerContent);
        doc.saveAndClose();
      } else {
        const doc  = DocumentApp.create(docName);
        const dId  = doc.getId();
      ...
      console.log('[generateDailyPrimer] ' + docName + ' saved to 03.1_CURRENT_STATE.');
    }
