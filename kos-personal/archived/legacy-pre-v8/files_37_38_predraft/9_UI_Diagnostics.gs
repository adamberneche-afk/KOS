...
function generateCouncilInputPayload() {
  ...
  try {
    ...
    body.appendParagraph(stateText.substring(0, CFG.MAX_CHUNK_SIZE));
    body.appendParagraph('2. THE LAWS (Active Constraints & Pivots)')
        .setHeading(DocumentApp.ParagraphHeading.HEADING2);
    body.appendParagraph(pivotText.substring(0, Math.floor(CFG.MAX_CHUNK_SIZE / 2)));
...
/**
 * Alias for buildSessionContext(). Resolves any external references
 * to the v5.4 Phase 4 function name generateSessionVectorPrimer().
 */
function generateSessionVectorPrimer() {
  buildSessionContext();
}


/**
 * Dev/testing helper. Manually advances a STAGING_PIPELINE row
 * from PENDING_FLOW or NEEDS_CURATOR to FLOW_COMPLETE, simulating
 * what Workspace Studio does when it has processed the payload.
 *
 * USE IN DEVELOPMENT ONLY. Run from the Apps Script editor.
 * Never called by any background trigger.
 *
 * @param {number} rowNumber  1-indexed row in STAGING_PIPELINE.
 *                            Row 2 = first data row (row 1 is header).
 */
function devSetFlowComplete(rowNumber) {
  const ui = _getUi();
  if (!rowNumber || rowNumber < 2) {
    ui.alert('devSetFlowComplete', 'rowNumber must be >= 2 (row 1 is the header).', ui.ButtonSet.OK);
    return;
  }
  const ss      = _getSystemAsset(CFG.INDEX_NAME, 'INDEX_ID', false);
  const staging = _getOrCreateSheet(ss, CFG.STAGING_SHEET);
  ...
  if (current !== 'PENDING_FLOW' && current !== 'NEEDS_CURATOR') {
    ui.alert('devSetFlowComplete',
      'Row ' + rowNumber + ' has status "' + current + '".\n\n' +
      'Only PENDING_FLOW or NEEDS_CURATOR rows can be promoted.',
      ui.ButtonSet.OK);
    return;
  }
  ...
  ui.alert('✅ Row ' + rowNumber + ' → FLOW_COMPLETE',
    'Run processInferenceQueue() next to process it.',
    ui.ButtonSet.OK);
}
