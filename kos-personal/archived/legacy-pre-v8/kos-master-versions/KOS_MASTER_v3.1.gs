 ...
 * Version: 5.4 | CI: 3.1 | Single-File Complete Deployment
 * Bound to: Drop Zone Document
 * ============================================================
 *
 * CHANGELOG CI 3.1 — Intake Pipeline Refactor:
 *   Phase 1 now chunks immediately after quarantine — no separate Phase 2
 *   Each chunk becomes its own Drive doc in 03.4_RAW_EXHAUST
 *   STAGING_PIPELINE schema: [Timestamp, Chunk_UID, Doc_URL, File_ID, Status]
 *   Inference_Buffer retired — Phase 3 reads VECTOR_MATRIX directly
 *   processInferenceQueue() replaces runIntakePipelineFromBuffer()
 *   exportChunksForCurator() replaces Phase 2 + Inference_Buffer workflow
 *   consolidateInferenceChunks() now primary: reads VECTOR_MATRIX for primer
 *
 * ── NEW SESSION CYCLE ─────────────────────────────────────────
 *   ① Intake & Chunk      — quarantine + chunk + create docs in one pass
 *   ② Export for Curator  — open chunk docs, feed to Curator, paste JSON back
 *   ③ Process Queue       — reads docs by File_ID, parses JSON, routes vectors
 *   ④ Consolidate Primer  — reads VECTOR_MATRIX, stores SESSION_VECTOR_PRIMER
 * ============================================================
 */
...
function onOpen() {
  ...
  ui.createMenu('🧠 Council')
    .addItem('① Intake & Chunk (Phase 1)',              'processManualSync')
    .addItem('② Export Chunks for Curator',             'exportChunksForCurator')
    .addItem('③ Process Inference Queue (Phase 4)',     'processInferenceQueue')
    .addItem('④ Consolidate Vector Primer (Phase 3)',   'consolidateInferenceChunks')
    .addSeparator()
  ...
  try {
    ...
    if (!cs.armed) {
      const go = ui.alert('⚠ Engine COLD',
        'No calibration weights set. Processing continues but vector scoring is inactive.\n\nContinue?',
        ui.ButtonSet.YES_NO);
    ...
    // Duplicate guard on full log UUID
    if (staging.getRange('B:B').getValues().flat().some(v => String(v).startsWith(logUUID))) {
      throw new Error('Duplicate Session: Log UUID already exists in STAGING_PIPELINE.');
    }
    ...
    rawDoc.saveAndClose();
    rawFile.moveTo(folder);
    rawFile.setSharing(DriveApp.Access.PRIVATE, DriveApp.Permission.EDIT);

    // Chunk immediately — one Drive doc per chunk in 03.4_RAW_EXHAUST
    const chunks = _semanticChunker(rawText);
    chunks.forEach((chunkText, idx) => {
      const chunkUID  = `${logUUID}_CH${(idx + 1).toString().padStart(2, '0')}`;
      const chunkName = `[CHUNK_${(idx + 1).toString().padStart(2, '0')}]_${logUUID}`;
      const chunkDoc  = DocumentApp.create(chunkName);
      const chunkFile = DriveApp.getFileById(chunkDoc.getId());
      chunkDoc.getBody().setText(chunkText);
      chunkDoc.saveAndClose();
      chunkFile.moveTo(folder);

      // Lightweight row — no text payload in the spreadsheet
      staging.appendRow([
        new Date(),
        chunkUID,
        chunkFile.getUrl(),   // Doc_URL (renders as Smart Chip in Sheets)
        chunkFile.getId(),    // File_ID for direct programmatic access
        'PENDING_INFERENCE',
      ]);
    });
    ...
    ui.alert(
      ...
      `All chunk docs quarantined in ${CFG.STAGING_FOLDER}.\n\n` +
      `NEXT:\n` +
      `② Export Chunks for Curator — open the docs and process through Curator Gem\n` +
      `③ Process Inference Queue — after pasting Curator JSON back into each doc`,
      ui.ButtonSet.OK
    );
...
// ══════════════════════════════════════════════════════════════
// ── PHASE 2 RETIRED ──────────────────────────────────────────
// Chunking now happens immediately in processManualSync (Phase 1).
// This stub is preserved so any saved references do not throw.
function processPhase2Chunking() {
  DocumentApp.getUi().alert(
    'Phase 2 Retired',
    'Chunking now happens automatically at the end of Phase 1.\n\n' +
    'Run ① Intake & Chunk to quarantine a log and create chunk docs in one step.',
...
// ══════════════════════════════════════════════════════════════
// PART 7: PHASE 4 (MENU) — CONSOLIDATE VECTOR PRIMER
// Inference_Buffer is retired. Primer is now read directly from
// the most recent VECTOR_MATRIX row written by routeVectorWeights().
// Falls back to legacy Inference_Buffer if matrix has no sessions.
// ══════════════════════════════════════════════════════════════
function consolidateInferenceChunks() {
  ...
  try {
    ...
    // Primary path — VECTOR_MATRIX populated by routeVectorWeights() in Phase 3
    if (matrix.getLastRow() > 1) {
      ...
      headers.slice(2).forEach((h, i) => {
        const v = parseFloat(lastRow[i + 2]);
        if (!isNaN(v) && h !== 'INCUBATOR_SIGNALS') weights[h] = v;
      });
      const primer = {
        consolidated_at: new Date().toISOString(),
        source:          'VECTOR_MATRIX',
        last_session:    sessionUid,
        vector_weights:  weights,
      };
      PropertiesService.getScriptProperties().setProperty('SESSION_VECTOR_PRIMER', JSON.stringify(primer));
      ui.alert('✅ Vector Primer Consolidated',
        `Source: VECTOR_MATRIX\nLast Session: ${sessionUid}\n\n` +
        Object.entries(weights).map(([k, v]) => `${String(k).padEnd(22)} ${v}`).join('\n') +
        '\n\nRun 🧠 Council → Build Session Context to inject into your next Gem session.',
        ui.ButtonSet.OK);
      return;
    }

    // Legacy fallback — Inference_Buffer rows if they still exist
    const buffer = _getOrCreateSheet(ss, CFG.INFERENCE_BUFFER_SHEET);
    if (buffer.getLastRow() > 1) {
      ...
      for (let i = 1; i < data.length; i++) {
        ...
        try {
          ...
          if (w && typeof w === 'object') {
            Object.entries(w).forEach(([t, v]) => {
              const s = parseFloat(v);
              if (!isNaN(s)) { if (!agg[t]) agg[t] = { sum:0, count:0 }; agg[t].sum += s; agg[t].count++; }
            });
          }
      ...
      if (done > 0) {
        const primer = { consolidated_at: new Date().toISOString(), source: 'INFERENCE_BUFFER_LEGACY', chunk_count: done, vector_weights: {} };
        Object.entries(agg).forEach(([t, d]) => { primer.vector_weights[t] = parseFloat((d.sum/d.count).toFixed(4)); });
        PropertiesService.getScriptProperties().setProperty('SESSION_VECTOR_PRIMER', JSON.stringify(primer));
        SpreadsheetApp.flush();
        ui.alert('✅ Primer Consolidated (Legacy)',
          `${done} chunk(s) from Inference_Buffer.${errs > 0 ? ` ⚠ ${errs} error(s).` : ''}\n\n` +
    ...
    ui.alert('Nothing to Consolidate',
      'VECTOR_MATRIX is empty and Inference_Buffer has no processable rows.\n\nComplete a full session cycle: ① → ② → ③ first.',
      ui.ButtonSet.OK);
...
// ══════════════════════════════════════════════════════════════
// ══════════════════════════════════════════════════════════════
// PHASE 4 — PROCESS INFERENCE QUEUE (formerly runIntakePipelineFromBuffer)
// Reads STAGING_PIPELINE, opens each chunk doc by File_ID, parses
// Curator JSON from the doc body, and calls processIntakePayload.
// Chunk docs start with raw text — they become ready for this function
// once the user replaces their content with Curator JSON output.
// ══════════════════════════════════════════════════════════════
function processInferenceQueue() {
  ...
  try {
    ...
    for (let i = 1; i < data.length; i++) {
      const status = data[i][4];
      if (status !== 'PENDING_INFERENCE' && status !== 'NEEDS_CURATOR') continue;

      const fileId = data[i][3]; // Column D — File_ID
      if (!fileId) {
        staging.getRange(i + 1, 5).setValue('ERROR: No File_ID in row');
        errs++;
        continue;
      }
      ...
      try {
        ...
        // Attempt JSON parse — if it fails the doc still has raw chunk text
        let parsed;
        try {
        ...
        staging.getRange(i + 1, 5).setValue(`ERROR: ${e.message}`);
    ...
    const notReadyMsg = notReady > 0
      ? `\n\n${notReady} chunk(s) still contain raw text (NEEDS_CURATOR).\nOpen them via ② Export Chunks for Curator, run each through the Curator Gem, paste the JSON back into the doc (replace all content), then re-run this function.`
      : '';
...
function runIntakePipelineFromBuffer() { processInferenceQueue(); }

/**
 * Shows all PENDING_INFERENCE and NEEDS_CURATOR chunk doc URLs from
 * STAGING_PIPELINE so the user can open them, process through Curator
 * Gem, and paste the JSON response back before running Phase 3.
 *
 * User workflow:
 *   1. Click a URL → chunk doc opens
 *   2. Copy the raw chunk text → paste into Curator Gem
 *   3. Copy the Curator JSON response
 *   4. Select All in the chunk doc → paste (replaces raw text with JSON)
 *   5. Save the doc
 *   6. Repeat for all chunks → run ③ Process Inference Queue
 */
function exportChunksForCurator() {
  ...
  try {
    ...
    const pending = data.slice(1).filter(r =>
      r[4] === 'PENDING_INFERENCE' || r[4] === 'NEEDS_CURATOR'
    );
    ...
    ui.alert(
      ...
      `  4. Return to the chunk doc → Select All → Paste (replace content with JSON)\n` +
...
function _getOrCreateSheet(ss, name) {
  ...
  const H = {
    [CFG.STAGING_SHEET]:         ['Timestamp','Chunk_UID','Doc_URL','File_ID','Status'],
