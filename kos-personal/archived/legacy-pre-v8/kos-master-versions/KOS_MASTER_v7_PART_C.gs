...
//   [ ] File saved in Apps Script editor
//   [ ] Open DROP_ZONE Google Doc → 🚀 Deploy → Deploy Full System
//   [ ] Fill in setupCalibration() → Run → Clear values
//   [ ] 🧠 Council → Setup Governance Trigger
//   [ ] 🧠 Council → Audit Calibration Health — confirm Engine ARMED
//   [ ] Open START_HERE_GEM_SETUP → configure Gemini Gem
//   [ ] Paste first session log → 🧠 Council → Process Session Log (Phase 1)
//   [ ] 🧠 Council → Trigger Partition (Phase 2)
//   [ ] 🧠 Council → Export Chunks for Curator (Phase 2.5) — review chunk list
//   [ ] Open BRAIN_TRUST_INDEX → Inference_Buffer tab
//       For each chunk: copy Col D text → Curator Gem → paste JSON back → set Col E = BUFFERED
