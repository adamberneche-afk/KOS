 ...
 * ============================================================================
 * CI: 1.1 | Three-Phase Execution Pipeline
 *
 ...
 *     Run AFTER setupRoutingProperties() confirms all 15 properties are set.
 *
 * CORRECT RUN ORDER:
...
function setupRoutingProperties() {
  ...
  const f03_raw = _findFolder("03.4_RAW_EXHAUST", f03);
  ...
  if (failCount > 0) {
    ...
    Logger.log("🟢 All clear. You can now run semanticRouterSweeper().");
  }
}
...
// ============================================================================
// CI: 1.1 | Enforces Idempotency | Uses PropertiesService Taxonomy Mapping
// Adheres to SMP-001 Direct-Routing Constraints
...
function semanticRouterSweeper() {
  ...
  try {
    ...
    // [FIX]: Use getFiles() instead of getFilesByType(MimeType.GOOGLE_DOCS)
    // getFilesByType only caught native .gdoc files — this now catches ALL types
    // (.gs, .md, .docx, Sheets, etc.) so no CE-tagged file is invisible to the sweep.
    const allFiles = DriveApp.getRootFolder().getFiles();

    // ==========================================================================
    // THE SEMANTIC TAXONOMY MAP (SMP-001)
    // Matches file prefix to permanent Google Drive Folder ID via PropertiesService
    // ==========================================================================
    const taxonomyMap = {
    ...
    while (allFiles.hasNext()) {
      ...
      // [FIX]: Null folder ID guard — surfaces misconfigured properties explicitly
      ...
      // ========================================================================
      // PHASE 3B: BLACKBOARD LEDGER (EXECUTION REGISTRATION)
      // ========================================================================
