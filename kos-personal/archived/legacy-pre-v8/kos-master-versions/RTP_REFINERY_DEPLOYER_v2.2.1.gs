 ...
 * CI: 2.2.1 | Bound to: Drop Zone Document
 * ============================================================
 *
 * CHANGELOG CI 2.2.1 (Bug Fixes):
 *   FIX: Phase 3 now processes BUFFERED and INTAKE_PROCESSED rows —
 *        previously Phase 3 and Phase 4 competed for BUFFERED status,
 *        whichever ran first won and the other found an empty queue.
 *   FIX: runSemanticSweeper() — moved DriveApp.getFilesByName() outside
 *        the while loop. Previously it made one Drive API call per file
 *        routed, causing quota exhaustion and execution timeouts.
 *   NEW: _reportError(context, error, ui) — unified error reporter that
 *        logs to console, shows an alert, AND emails the active user.
 *        Wired into every catch block across the entire script.
