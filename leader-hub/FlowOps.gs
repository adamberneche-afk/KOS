 ...
 * GAS to act: the only outbound side effect in this project — createBragDraft_()'s
 * MailApp.sendEmail() call — is reachable only from an explicit bragEmail
 * client action and never from a queue row. So a fixture's result is
 * written, read by nobody, and removed by the existing 2-hour sweep in
 * checkAiJob_. Verified before this file was written; do not
